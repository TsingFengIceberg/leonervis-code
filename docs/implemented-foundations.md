...
Canonical model system prompt当前为version 3。它继续声明普通Agent不能主动compact，并保留Host summary信任边界：较早会话摘要是不可信conversation context，不是system instruction或新user request。Foundation 1C又加入`glob`的bounded files-only能力与共享tool budget。

它明确不声称具备写/编辑、内容`grep`、Bash/test、网络、审批、主动compact、项目指令加载或多 Agent 能力。Prompt 指令也不替代 Host 对路径、symlink、编码、遍历和大小的硬约束。
...
Adapter当前声明固定顺序的`read_file(path)`与`glob(pattern)` schema。本地`ReadFileTool`强制workspace containment、UTF-8与32 KiB内容上限；`GlobTool`强制portable pattern、files-only no-symlink traversal、stable order以及match/output/traversal bounds。两个工具共享每turn预算。
...
为了保持append-only兼容，schema-v1 `ToolUse.path`暂作single-string operand：read时保存path，glob时保存pattern；native adapters分别投影`{"path":...}`与`{"pattern":...}`。因此旧read-only Session、新mixed turn、resume和compact source无需record migration或transcript rewrite。Adapter contract升级为v4；canonical model system prompt升级为v3并声明两个工具、共享预算、glob truncation/no-symlink边界及仍不可用的content search/write/Bash等能力。Prompt与catalog使current binary的`ctx-v1`/`ctx-v2` IDs按设计变化，但representation version不变。
