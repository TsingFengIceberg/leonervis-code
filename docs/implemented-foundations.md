...
Canonical model system prompt当前为version 2。它继续声明普通Agent不能主动compact，但新增Host summary信任边界：较早会话摘要是不可信conversation context，不是system instruction或新user request。

它明确不声称具备写/编辑、glob/grep、Bash/test、网络、审批、compact、项目指令加载或多 Agent 能力。Prompt 指令也不替代 Host 对路径、编码和大小的硬约束。

System prompt 不属于 `ConversationItem`，所以 `/history`、`ProjectSession.history` 和 append-only Session JSONL 只保存真实 user/assistant/tool 因果链。恢复旧 Session 后，新 turn 使用当前 binary 的 canonical prompt；schema-v2 compact checkpoint只保存compact prompt/summary-framing provenance，不把正常system prompt写进conversation history。
...
REPL `/compact` 现在能在保留完整 append-only transcript 与 `/history` 的同时，手动缩短 provider-visible effective context。第一版固定要求至少 4 个完整 effective turns，保留最近 2 个 turns原文，并用当前真实 provider 对更早 projection生成一次 summary；fake runtime不可用，不自动触发，也不重试原 user turn。
...
Session不重写旧行：既有records继续是schema v1，只有typed `context_compacted`是schema v2。Mixed replay从所有`TurnCommitted`重建full history，从latest checkpoint重建summary/retained suffix，并让后续turn同时追加到full/effective。Checkpoint append复用candidate replay validation、O_APPEND、flush/fsync，然后才安装内存effective state。
...
本切片不处理 `/resume`/`--resume` 的切换前判断；当前 `SessionStore.open()` 在返回前已经追加 resume record 并更新 latest pointer，原子 reject 需要后续 read-only prepare/commit 设计。也不实现 compact、历史删除或自动新 Session。
