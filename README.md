...
> **当前状态：** 已支持命名 provider profile、真实/离线 runtime、可恢复 Session、受限 `read_file` 工具循环、provider-owned 模型限制、target-specific preflight、切换前 screening、provider-neutral Effective Context、手动且可恢复的 `/compact`、target-aware startup/REPL resume prepare/screen/commit，以及固定 80% high-water 与 known overflow 触发的 pre-turn automatic compact。尚未实现写工具、Bash 或审批流程。
...
当前仅提供一个 workspace-bound `read_file` 工具；尚无写/编辑、glob/grep、Bash/test、网络工具、审批、streaming、自动 retry/fallback、并行工具、多 Agent 或远程服务。

Foundation 3H已把3F-2的Controlled Compact Transaction提升为pre-turn orchestration：exact initial request在known window达到固定80%或known context overflow时最多自动尝试一次，pending user只参与fit assessment而不进入summary/checkpoint，proactive failure可继续原known-fit turn，mandatory failure不发送普通generation。下一步建议回到v0工具面，选择一个更小的只读vertical slice（优先`glob`/文件匹配），继续沿用workspace硬边界、结构化结果和确定性测试；provider-error retry、write/Bash/approval仍保持独立后续切片。完整范围、开发原则和路线记录在 [CLAUDE.md](./CLAUDE.md) 与各 ADR 中。
