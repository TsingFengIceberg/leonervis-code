...
> **当前状态：** 已支持命名 provider profile、真实/离线 runtime、可恢复 Session、受限 `read_file` 工具循环、provider-owned 模型限制、target-specific preflight、切换前 screening、provider-neutral Effective Context、手动且可恢复的 `/compact`，以及 target-aware startup/REPL resume prepare/screen/commit。尚未实现自动 compact、写工具、Bash 或审批流程。
...
当前仅提供一个 workspace-bound `read_file` 工具；尚无写/编辑、glob/grep、Bash/test、网络工具、审批、streaming、自动 retry/fallback、并行工具、自动 compact、多 Agent 或远程服务。

Foundation 3G 已让 startup 与 REPL resume 在任何审计/pointer写入前重放并screen目标 Effective Context，并以descriptor lease、exact stale/CAS、same-current no-op和typed partial outcomes保证存储语义可审计。下一步建议回到 v0 工具面，选择一个更小的只读 vertical slice（优先 `glob`/文件匹配），继续沿用 workspace 硬边界、结构化结果和确定性测试；自动 compact/retry、write/Bash/approval 仍保持独立后续切片。完整范围、开发原则和路线记录在 [CLAUDE.md](./CLAUDE.md) 与各 ADR 中。
