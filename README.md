...
> **当前状态：Foundation 1A 的内存文本历史已完成。** 不带子命令启动时会进入本地交互终端；同一 REPL 进程中的后续输入会看到此前已完成的 user/assistant 文本对，尚不是可执行真实 Agent 任务的运行时。
...
启动后会显示彩色 LEO 标志、版本、当前目录和 Foundation 1A 状态，然后显示：
...
## Foundation 1A：确定性 prompt、REPL 与内存历史

当前 REPL 和 `prompt` 命令只完成以下最小、可测路径：

```text
终端输入 → AgentLoop（有序内存历史）→ ScriptedFakeProvider → 文本输出
```

每条非空输入恰好执行一次 provider 调用，并原样显示其返回的文本。REPL 在一个运行进程中保留有序的文本历史：第二次调用 provider 时会收到第一次成功的 user/assistant 对以及新的 user 输入。`prompt` 命令仍是一次性接口，每次调用都从空历史开始；每次新启动 REPL 也从空历史开始。

这些历史只保存在当前进程内，可通过 REPL 的 `/history <count>` 按回合查看，但既不写入磁盘，也不是 session、transcript、恢复或长期记忆。

这一切片**不会**调用模型 API、读取凭据或环境变量、访问网络、执行文件或 Bash 工具、写入 session、读取 workspace，也不提供真实模型、审批或持久化。若在非交互终端中直接运行 `leonervis-code`，程序会提示改用 `leonervis-code prompt "..."` 并以非零状态退出，避免管道或 CI 意外卡住。

详细的学习设计记录见：[单轮 Loop 决策](./docs/decisions/0001-foundation-0-single-turn-loop.md) 与 [确定性 REPL 决策](./docs/decisions/0002-foundation-0-deterministic-repl.md)。
...
- `TextMessage` / `ConversationProvider` contract、确定性 scripted fake provider 与携带内存历史的 `AgentLoop`；
- 具有彩色启动标志、最小控制命令、Tab 补全和有序进程内文本历史的本地 REPL；
- 可通过 `prompt` 命令端到端运行的自动化友好路径；
- `pytest` 与 `ruff` 的基础质量工具链。

下一切片可在保留文本因果链的前提下，引入更明确的 assistant content contracts 与受限的工具循环。真实模型接入、文件工具、写入审批、session 与受控 Bash 都仍需各自的学习切片。
...
```text
src/leonervis_code/
  core/                 # 中立文本 contracts：TextMessage 与 ConversationProvider
  agent/                # 维护有序内存文本历史的 AgentLoop
