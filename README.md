...
> **当前状态：** 已支持命名 provider profile、真实/离线 runtime、可恢复 Session、受限 `read_file` 工具循环、provider-owned 模型限制、每次 provider invocation 前的 target-specific preflight、切换前 screening，以及 provider-neutral Effective Context Snapshot 与只读 `/context` 检查。尚未实现 compact、写工具、Bash 或审批流程。
...
Ctrl-D、EOF 或在等待输入时按 Ctrl-C 也会正常退出。`/context` 不调用模型生成、不修改 Session，也不写 transcript；当前 source 是完整 committed history，full/effective history 尚未分离。Anthropic official route 的 exact inspection 可能发起一次 count-only `messages.count_tokens` 请求，OpenAI-compatible 使用本地 estimate。终端颜色只在 TTY 中启用；设置 `NO_COLOR=1` 可关闭。
...
下一切片计划实现 controlled compact transaction：在完整 transcript 保持 append-only 的前提下，为 effective history 引入 typed durable checkpoint、stale-source conflict check 与失败原子性。Target-aware resume prepare/commit 仍作为独立切片。完整范围、开发原则和路线记录在 [CLAUDE.md](./CLAUDE.md) 与各 ADR 中。
