...
> **当前状态：Foundation 3C 的命名 Provider Profiles 与常驻运行时已完成。** Anthropic、OpenAI、xAI、DashScope/Qwen/Kimi、Ollama/local、OpenRouter 和受控 custom endpoint 现在既可用一次性 `--model` 调用，也可保存为不含 secret value 的全局 profile，并由 workspace 选择 active profile。`ProjectSession` 为其他模块提供同一套常驻 client、多轮中立历史与 turn 间原子切换 API；裸 REPL 在存在显式/active profile 时进入真实对话，没有任何选择时仍使用确定性 fake/offline provider。两个真实 SDK 均固定 `max_retries=0`。
...
REPL 内提供进程内历史与 provider runtime 控制命令：

```text
/help                         查看控制说明
/history <count>              显示最近 count 个完整对话回合
...
profile 定义保存在 `${XDG_CONFIG_HOME:-~/.config}/leonervis-code/providers.json`；workspace 只在 `.leonervis-code/provider.json` 保存 active profile 名。两个 JSON 都不保存 key value；项目目录是本地运行状态，应加入目标项目的 `.gitignore`。
...
`ProjectSession` 还提供 `list_profiles()`、`use_profile()`、`clear_active()`、`status()`、`history` 与 `turns`。Foundation 3C 的历史仍只存在当前进程；JSONL transcript 与 resume 留给 Foundation 3D。
...
显式 provider namespace 优先；只有已登记的 `claude-*`、`gpt-*`、`grok-*`、`qwen-*`、`kimi-*` bare family 会被确定性识别，未知 bare model 不依据现有 credential 猜测。route 和 adapter config 不保存 secret value；key 只在 factory 构造所选 SDK client 时读取。当前不读取 `.env`、OAuth 或 keyring，也不实现 streaming、自动 retry/backoff、fallback execution、live discovery、并行工具或磁盘会话恢复。
...
- 不含 credential value 的命名 provider profiles、user/project active precedence、turn 间原子 client/model 切换，以及供其他模块使用的 `ProjectSession` API；
- 具有彩色启动标志、Tab 补全、`/history`、provider/status/model 控制和有序进程内完整回合历史的本地真实/fake REPL；
- 可通过 `prompt` 命令端到端运行的自动化友好路径；
- `pytest` 与 `ruff` 的基础质量工具链。

下一切片建议进入 Foundation 3D：在这里已验证的 `ProjectSession` 和 provider provenance 上加入 append-only JSONL transcript、稳定 session identity 与 resume，同时保持工具因果链成对持久化。Streaming、自动 retry/fallback、文件写入、审批和受控 Bash 都仍需各自的学习切片。
...
src/leonervis_code/
  ...
  session.py            # 面向项目其他模块的 ProjectSession facade
