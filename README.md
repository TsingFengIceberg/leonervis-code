...
> **当前状态：第一版 provider-neutral canonical model system prompt 已完成。** 每个 fake/Anthropic/OpenAI-compatible 模型请求现在都携带同一个版本化、可指纹验证的稳定模型契约；Anthropic 使用 top-level `system`，compatible Chat Completions 使用单个 leading system message。Prompt 只陈述当前受限 `read_file` 能力，不注入 workspace 绝对路径、Session/provider/model 或 credential，也不进入 conversation history 与 Session schema v1。恢复 Session 后的新 turn 使用当前 binary 的 canonical prompt；历史 turn 的 exact prompt provenance 暂未持久化。Profile identity、append-only JSONL、persist-before-memory、跨进程 `--resume` 和 runtime/history 解耦保持不变，两个真实 SDK 仍固定 `max_retries=0`。
...
## 第一版 canonical model system prompt

Leonervis Code 现在从 `src/leonervis_code/system_prompt.py` 构建一个 provider-neutral `SystemPromptSnapshot`，包含显式版本、规范化文本和 domain-separated SHA-256 fingerprint。它在每个 user turn 开始时构建一次，并在该 turn 的所有 `read_file` continuation 中固定不变：
...
第一版 prompt 是稳定契约，不含 workspace 绝对路径、日期、Session ID、provider/model/profile、endpoint 或 credential。它只告诉模型当前确实能做的事情：选择性读取 workspace 内一个相对路径的 UTF-8 文本文件、接受 bounded/truncated tool result，并基于真实结果回答。它明确不声称具备写/编辑、glob/grep、Bash/test、网络、审批、compact、项目指令加载或多 Agent 能力；prompt 指令也不替代 Host 对路径、编码和大小的硬约束。

System prompt 不属于 `ConversationItem`，所以 `/history`、`ProjectSession.history` 和 append-only Session JSONL 仍只保存 user/assistant/tool 因果链。恢复旧 Session 后，新 turn 使用当前 binary 的 canonical prompt；Session schema v1 尚不保存历史 turn 的 exact prompt version/fingerprint，这一 audit 缺口留待未来独立 schema migration。这里的 **model system prompt** 与终端中的 `leonervis[session8|runtime]>` **REPL prompt** 是两个完全不同的界面。

详细设计见 [第一版 canonical model system prompt 决策](./docs/decisions/0012-first-canonical-model-system-prompt.md)，Claw-Code prompt 结构学习入口见 [docs/references/claw-code-prompts](./docs/references/claw-code-prompts/README.md)。
...
```text
终端输入 → AgentLoop（固定一个 canonical system prompt snapshot + 有序内存因果上下文）
...
详细学习设计记录见：[单轮 Loop 决策](./docs/decisions/0001-foundation-0-single-turn-loop.md)、[确定性 REPL 决策](./docs/decisions/0002-foundation-0-deterministic-repl.md)、[内存文本历史决策](./docs/decisions/0003-foundation-1a-in-memory-text-history.md)、[受限 read-file 工具循环决策](./docs/decisions/0004-foundation-1b-bounded-read-file-tool-loop.md)、[provider-neutral 模型路由决策](./docs/decisions/0005-foundation-2a-provider-neutral-model-routing.md)、[adapter-owned compatibility policy 决策](./docs/decisions/0006-foundation-2b-adapter-owned-compatibility-policy.md)、[Anthropic 非流式 adapter 决策](./docs/decisions/0007-foundation-3a-anthropic-non-streaming-adapter.md)、[本地多 Provider runtime 决策](./docs/decisions/0008-foundation-3b-local-multi-provider-runtime.md)、[命名 provider profile/常驻 runtime 决策](./docs/decisions/0009-foundation-3c-named-provider-profiles-and-runtime-manager.md)、[稳定 profile/可恢复 Session 决策](./docs/decisions/0010-foundation-3d-stable-profile-identity-and-durable-sessions.md)、[解耦 REPL 展示/命令分发决策](./docs/decisions/0011-decoupled-repl-presentation-and-slash-dispatch.md) 与 [第一版 canonical model system prompt 决策](./docs/decisions/0012-first-canonical-model-system-prompt.md)。
...
- 结构化的 `SystemPromptSnapshot` / `ConversationRequest` / `UserMessage` / `AssistantText` / `ToolUse` / `ToolResult` contract、版本化且可指纹验证的稳定 model system prompt、确定性 scripted fake provider、携带原子内存因果历史的 `AgentLoop`、一个受限的 `read_file` 工具、Foundation 2B 离线 route policy，以及支持 Anthropic 与 OpenAI-compatible family 的本地多 Provider runtime；
...
src/leonervis_code/
  ...
  system_prompt.py      # canonical model system prompt、版本与 fingerprint
  session.py            # 组合 runtime 与可切换 durable Session 的 ProjectSession facade
