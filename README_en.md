...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled writes end to end; Foundation 4B adds unique exact edit; Foundation 4C now adds controlled local command execution. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, delete, and mkdir remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, and direct-argv `run_command`; all six share at most three sequential calls per user turn. Commands require `danger-full-access` and support ask/auto approval, a 1–300 second timeout, separate 32 KiB stdout/stderr captures, base64 for non-UTF-8 output, process-group cleanup, and the same durable Action Audit. This is not shell source, a PTY, or a sandbox: test code may still access outside the workspace, use the network, read filesystem credentials, or start child processes, and side effects cannot be promised rollback.

Foundation 4C is complete. The canonical system prompt is v7 and the provider adapter contract is v8; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Regex/index/ignore-aware search, fuzzy or multi-edit patching, delete, mkdir, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. The next independent direction should first design controlled `mkdir`/delete/rename path, approval, and failure-atomicity boundaries rather than inserting arbitrary Bash text into the command contract. See [ADR 0028](./docs/decisions/0028-foundation-4c-controlled-command-contract-and-preparation.md), [ADR 0029](./docs/decisions/0029-foundation-4c-bounded-command-execution-and-process-cleanup.md), and [ADR 0030](./docs/decisions/0030-foundation-4c-durable-model-visible-command-integration.md).
