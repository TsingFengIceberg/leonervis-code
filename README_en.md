...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob` loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual resumable `/compact`, target-aware startup/REPL resume prepare/screen/commit, and pre-turn automatic compaction triggered by a fixed 80% high-water mark or known overflow are implemented. Content search, write tools, Bash, and approval flows are not yet implemented.
...
- [Bounded Workspace Glob](./docs/decisions/0020-foundation-1c-bounded-workspace-glob.md): portable patterns, hidden/symlink policy, deterministic bounds, the shared tool budget, and the schema-v1 compatibility seam.
...
The current workspace-bound tool surface consists of bounded read-only `read_file` and `glob`. There is no content-search `grep`, write/edit, Bash/test, network, approval, streaming, automatic retry/fallback, parallel-tool, multi-agent, or remote-service capability yet.

Foundation 1C completes the minimum file-discovery path: the model can use a portable bounded glob to match candidate regular files, then use `read_file` for contents. Both tools share a three-call sequential budget while preserving structured causality, Session replay, and workspace hard boundaries. The recommended next independent slice is bounded content `grep`, with line/result limits, encoding, binary, and ignore semantics designed first; write/Bash and approval remain deferred. [CLAUDE.md](./CLAUDE.md) and the ADRs record the complete scope, principles, and roadmap.
