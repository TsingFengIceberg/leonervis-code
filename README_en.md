...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file` loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled create/overwrite end to end. Foundation 4B completes unique exact replacement as the fifth model-visible tool. Bash, regex/fuzzy patching, delete, and mkdir remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, and unique exact `edit_file`; all five share at most three sequential calls per user turn. Regex/index/ignore-aware search, fuzzy or multi-edit patching, delete, mkdir, Bash/test, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable.

Foundation 4B is now complete. Slices 0–3 establish the controlled exact-edit contract, side-effect-free preparation, failure-atomic executor, and authorization/audit composition; Slice 4 connects it to the canonical catalog, both provider projections/parsers, the system prompt, Effective Context identity, and ProjectSession dispatch. It only changes an existing strict UTF-8 regular file of at most 1 MiB, requires `old_text` to occur uniquely, and builds one complete candidate. Success preserves mode and atomically replaces the target; pre-replace failures leave it unchanged, while unknown directory durability is truthfully recorded as partial. The canonical system prompt is now v6 and the adapter contract is v7; the five model-visible tools retain the shared three-call sequential budget. ToolArguments v1, new `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. The next independent direction should first design controlled command/test execution action classes, arguments and output bounds, timeout, approval, and failure atomicity rather than exposing arbitrary Bash directly. The tracked ADRs and implemented-foundations documents are authoritative for the full boundaries.
