...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command`/`mkdir` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled writes end to end; Foundation 4B adds unique exact edit; Foundation 4C adds controlled local command execution, and Foundation 4D now adds controlled single-directory creation. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, delete, and rename remain unavailable.
...
The default is `read-only + ask`. `--permission-mode` is the capability ceiling, while `--approval` decides whether an in-scope action asks or proceeds automatically; the controls are independent. A one-shot `prompt` fail-safely cancels an `ask` and never reads stdin for approval. Only the REPL presents a write or directory path, a write byte count, or a command argv/cwd/timeout, accepting `y/yes`, `n/no`, or `c/cancel`. `run_command` is always `dangerous`: both `read-only` and `workspace-write` deny it, and only `danger-full-access` may continue through ask/auto.
...
`edit_file(path, old_text, new_text)` is now the fifth model-visible tool for one unique exact replacement in an existing UTF-8 file. It reuses `workspace-overwrite` approval, source SHA-256 revalidation, atomic replacement, and durable Action Audit, and rejects zero matches, multiple matches including overlaps, no-ops, symlinks, and stale sources. `new_text` may be empty for exact deletion. It creates neither files nor parent directories and does not support regex, fuzzy matching, or replacing multiple occurrences at once.

`run_command(argv, cwd, timeout_seconds)` is the sixth model-visible tool for tests, lint, and build verification. The Host executes argv directly rather than parsing shell source, closes stdin, requires an existing non-symlink workspace-relative cwd, enforces a 1–300 second timeout, and retains at most 32 KiB from each of stdout and stderr. Commands inherit only a fixed Host environment allowlist and receive bounded TERM-to-KILL process-group cleanup after timeout, cancellation, or lingering descendants. This is not an OS, filesystem, network, or credential sandbox, and already-visible side effects cannot be rolled back.

`mkdir(path)` is the seventh model-visible tool and creates exactly one missing workspace-relative directory. It is always `workspace-create`: `read-only` denies it, while `workspace-write` or `danger-full-access` proceeds through ask/auto policy. The parent must already exist and no path component may be a symlink. It neither creates parents recursively nor treats an existing directory as success. A target that appears while approval is pending is stale; successful creation fsyncs the new directory and its parent, while a visible directory whose durability cannot be confirmed returns a partial result that must not be retried automatically.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, direct-argv `run_command`, and single-directory `mkdir`; all seven share at most three sequential calls per user turn. Commands still require `danger-full-access`. Mkdir is `workspace-create`, requires an existing parent, rejects symlinks and stale targets, never creates parents recursively, and shares PermissionGate, approval, and durable Action Audit with writes and commands.

Foundation 4D is complete. The canonical system prompt is v8 and the provider adapter contract is v9; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Regex/index/ignore-aware search, fuzzy or multi-edit patching, delete, rename, recursive mkdir, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. The recommended next independent direction is controlled non-overwrite rename, followed separately by file-only delete and empty-directory removal, without introducing recursive deletion. See [ADR 0031](./docs/decisions/0031-foundation-4d-controlled-single-directory-creation.md).
